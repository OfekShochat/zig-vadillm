...
pub fn calcOne(
    ...
    // ranges are intra-block
    const block = func.getBlockAt(live_range.start);
    ...
    // var operands = std.ArrayList(Operand).init(allocator);
    // const insts = func.insts[live_range.start.toArrayIndex()..live_range.end.toArrayIndex()];

    // use-frequency bonus
    bonus += live_range.uses.len * 10 / (live_range.rawEnd() - live_range.rawStart());
